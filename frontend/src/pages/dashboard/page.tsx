...
import { estadoUrgenciaConfig, type Urgencia } from '@/mocks/urgencias';
      ...
      <Card padding="lg">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {triageEsperaDist.map((item) => {
            const label = legacyMockTriageLabel(item.nivel);
            const badge = legacyMockTriageBadgeClass(item.nivel);
            ...
            return (
              <button
                ...
                className={`text-left p-4 rounded-xl border transition-all cursor-pointer ${
                  item.count > 0 ? 'border-secondary-200 bg-background-50 hover:shadow-sm' : 'border-secondary-100 bg-background-50'
                }`}
              >
                <div className="flex items-center gap-2 mb-2">
                  <span className={`text-2xs px-2 py-0.5 rounded-full font-semibold ${badge}`}>
                    {label}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 mt-1.5">
                  <div className="flex-1 h-1.5 bg-secondary-200 rounded-full overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all duration-500 bg-secondary-400"
                      style={{ width: `${pct}%` }}
                    ></div>
                  </div>
...
function UrgenciaRow({ urgencia, onClick }: { urgencia: Urgencia; onClick: () => void }) {
  const triageLabel = legacyMockTriageLabel(urgencia.nivelUrgencia);
  const triageBadge = legacyMockTriageBadgeClass(urgencia.nivelUrgencia);
  ...
  return (
    <button onClick={onClick} className="w-full text-left flex items-center gap-4 px-5 py-3 hover:bg-secondary-50 transition-base cursor-pointer">
      <div className={`w-3 h-3 rounded-full flex-shrink-0 bg-secondary-400 ${urgencia.estado === 'en_atencion' ? 'animate-pulse' : ''}`}></div>
      ...
      <div className="text-right flex-shrink-0">
        <div className="flex items-center gap-1.5 justify-end">
          ...
          <span className={`text-2xs px-1.5 py-0.5 rounded-full ${triageBadge}`}>{triageLabel}</span>
        </div>
