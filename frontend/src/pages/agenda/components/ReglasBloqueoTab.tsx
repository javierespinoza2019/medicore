...
import type { ProfessionalDto, SpecialtyDto } from '@/api/professionals';
...
function describeRegla(
  ...
  if (r.tipo === 'medico') {
    ...
    return `${r.fecha} · ${d?.fullName.split(' ').slice(0, 2).join(' ') || 'Médico'}`;
  }
        ...
        <div className="p-4 bg-secondary-50 border border-secondary-200 rounded-xl space-y-3">
          ...
          {tipo === 'especialidad' && (
            <Select
              ...
              options={specialties.map((s) => ({ value: s.specialtyId, label: s.name }))}
            ...
            <div key={r.id} className={`flex items-center gap-3 px-4 py-3 rounded-lg border ${r.activo ? 'border-secondary-200 bg-background-50' : 'border-secondary-200 bg-secondary-50 opacity-70'}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-foreground-500 truncate">
                  {describeRegla(r, professionals, specialties)}
                </p>
              </div>
