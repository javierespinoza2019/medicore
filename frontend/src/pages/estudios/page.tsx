...
export default function Estudios() {
  ...
  useEffect(() => {
    // Solo la primera vez que llega el parámetro: no reabrir ni limpiar filtros
    // cada vez que cambia la lista de estudios.
    if (estudioParam && estudioSeleccionado?.id !== estudioParam) {
  ...
  }, [estudioParam, estudiosList, estudioSeleccionado]);
  ...
  // Sólo el id que trajo el servidor. Sin profesional → fail closed.
  const myDoctorId = user?.doctorId?.trim() || null;

  const estudiosFiltrados = useMemo(() => {
    let list = [...estudiosList];
    // Médico sin profesional asociado: no ve estudios ajenos (fail closed).
    if (isDoctor) {
      list = myDoctorId ? list.filter((e) => e.doctorId === myDoctorId) : [];
    }
    if (pacienteParam) {
  ...
  }, [search, filterEstado, filterTipo, pacienteParam, estudiosList, isDoctor, myDoctorId]);
  ...
  const changeEstado = (newEstado: EstudioSolicitado['estado']) => {
    ...
    // 'completado' sale por el modal de resultado, que es quien fija `fechaResultado`.
    const updated = {
      ...estudioSeleccionado,
      estado: newEstado,
    };
