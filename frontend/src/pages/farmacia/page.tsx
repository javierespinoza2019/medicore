...
import { legacyMockTriageLabel, legacyMockTriageBadgeClass, legacyMockTriageSortIndex } from '@/utils/legacyMockTriagePresentation';
...
export default function Farmacia() {
  ...
  const getUrgenciaPriorityLevel = useCallback((urgenciaId: string): number => {
    ...
    if (!urg) return 99;
    return legacyMockTriageSortIndex(urg.nivelUrgencia);
  }, []);

  const getUrgenciaNivel = useCallback((urgenciaId: string): string | null => {
    const urg = getUrgenciasGlobal().find((u) => u.id === urgenciaId);
  ...
  }, [recetasState, search, tabFarmacia, getUrgenciaPriorityLevel]);
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {isUrgencia && nivelUrg ? (
                            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-2xs font-medium border border-secondary-200 ${legacyMockTriageBadgeClass(nivelUrg)}`}>
                              {legacyMockTriageLabel(nivelUrg)}
                            </span>
                          ) : r.estado === 'parcial' ? (
                      ...
                      <span className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                        <i className={`text-sm ${r.estado === 'parcial' ? 'text-amber-500' : 'text-foreground-400'} ${selected ? 'ri-arrow-up-s-line' : 'ri-arrow-down-s-line'}`}></i>
                      </span>
                    </div>
                  </button>
                );
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {recetaSeleccionada.urgenciaId && (() => {
                          ...
                          return niv ? (
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-2xs font-medium border border-secondary-200 ${legacyMockTriageBadgeClass(niv)}`}>
                              {legacyMockTriageLabel(niv)}
                            </span>
                          ) : null;
                        })()}
