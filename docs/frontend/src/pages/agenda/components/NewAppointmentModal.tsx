...
export default function NewAppointmentModal({ open, onClose, defaultDate, onCreateAppointment, allAppointments, defaultPatientId, defaultTime, lockDateTime = false, consultorios, defaultConsultorioId }: NewAppointmentModalProps) {
  ...
  const filteredDoctors = useMemo(() => {
    let list = doctors.filter((d) => d.status === 'activo');
    if (consultorioId) {
      const cons = consultorios.find((c) => c.id === consultorioId);
      if (cons) list = list.filter((d) => cons.medicosIds.includes(d.id));
    } else if (specialtyFilter) {
      list = list.filter((d) => d.especialidad === specialtyFilter);
    }
    return list;
  }, [consultorioId, consultorios, specialtyFilter]);
  ...
  const handleCreate = () => {
    ...
    const newAppointment: Appointment = {
      ...
      estado: 'reservada',
      motivo,
      consultorio: selectedConsultorio?.nombre || selectedDoctor.consultorio,
    };
          ...
          <div className="space-y-4">
            ...
            {consultorioId && selectedConsultorio ? (
              <div className="p-3 bg-secondary-50 rounded-lg border border-secondary-200">
                ...
                <p className="text-sm font-semibold text-foreground-900">{specialties.find((s) => s.id === selectedConsultorio.especialidadId)?.nombre || '—'}</p>
              </div>
            ) : (
              <Select
                ...
                options={specialties.map(s => ({ value: s.nombre, label: s.nombre }))}
            ...
            <Select
              ...
              options={filteredDoctors.map(d => ({ value: d.id, label: `${d.nombre} — ${d.consultorio}` }))}
              value={doctorId}
            ...
            <div className="p-4 bg-secondary-50 rounded-xl space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm pt-2 border-t border-secondary-200">
                <div>
                  ...
                  <p className="font-medium text-foreground-900">{selectedDoctor.nombre}</p>
                </div>
                <div>
                  <p className="text-xs text-foreground-400">Especialidad</p>
                  <p className="font-medium text-foreground-900">{selectedDoctor.especialidad}</p>
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <p className="font-medium text-foreground-900">{selectedDoctor.consultorio}</p>
                </div>
                {motivo && (
                  <div className="col-span-2">
