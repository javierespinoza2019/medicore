...
function ConsultorioColumn({
  ...
  }, [onScheduleAtTime, date, blockedSlots, column.consultorioId, timeGranularity]);
  ...
  const columns = useMemo<ColumnDef[]>(() => {
    const cols: ColumnDef[] = activeConsultorios.map((c) => {
      ...
      const subtitulo = [
        ...
        linkedDoctors.map((d) => d.nombre.split(' ').slice(0, 2).join(' ')).join(', '),
      ].filter(Boolean).join(' · ');
      return {
        key: c.id,
        titulo: c.nombre,
    ...
    return cols;
  }, [activeConsultorios, activeNames, appointments]);
