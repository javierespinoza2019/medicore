...
import { doctors, specialties } from '@/mocks/doctors';
import { patients } from '@/mocks/patients';
import { appointments as initialAppointments, statusConfig, type Appointment } from '@/mocks/appointments';
...
import { consultorios as initialConsultorios } from '@/mocks/consultorios';
import { reglasBloqueo as initialReglasBloqueo } from '@/mocks/agendaRules';
...
function getWeekDays(mondayStr: string): { date: string; dayName: string; dayNumber: string; isToday: boolean }[] {
...
function getAppsByDate(appointments: Appointment[], date: string) {
...
export default function Agenda() {
  ...
  const [consultorios, setConsultorios] = useState(initialConsultorios);
  const [reglasBloqueo, setReglasBloqueo] = useState(initialReglasBloqueo);
  ...
  const filteredPatient = useMemo(() => patients.find(p => p.id === urlPatientId), [urlPatientId]);
  ...
  const filteredAppointments = useMemo(() => {
    let apps = view === 'day'
      ? getAppsByDate(localAppointments, selectedDate)
      : view === 'week'
        ? localAppointments.filter(a => a.fecha >= weekDays[0].date && a.fecha <= weekDays[6].date)
        : localAppointments;

    // Filtrar por sucursal actual
    if (sucursalActualId) apps = apps.filter(a => a.sucursalId === sucursalActualId);
    ...
    if (filterSearch.trim()) {
      ...
      apps = apps.filter(a => a.patientName.toLowerCase().includes(q) || a.doctorName.toLowerCase().includes(q) || a.motivo.toLowerCase().includes(q));
    }
    return apps;
  }, [localAppointments, view, selectedDate, filterDoctor, filterSpecialty, filterStatus, filterSearch, weekDays, urlPatientId, sucursalActualId]);

  const stats = useMemo(() => {
    let dayApps = getAppsByDate(localAppointments, selectedDate);
    if (sucursalActualId) dayApps = dayApps.filter(a => a.sucursalId === sucursalActualId);
    return {
      total: dayApps.length,
      atendidas: dayApps.filter(a => a.estado === 'atendida').length,
      enCurso: dayApps.filter(a => a.estado === 'en_consulta' || a.estado === 'en_espera' || a.estado === 'llego').length,
      confirmadas: dayApps.filter(a => a.estado === 'confirmada' || a.estado === 'reservada').length,
      canceladas: dayApps.filter(a => a.estado === 'cancelada' || a.estado === 'no_acudio').length,
    };
  }, [localAppointments, selectedDate, sucursalActualId]);

  const activeFiltersCount = [filterDoctor, filterSpecialty, filterStatus].filter(Boolean).length;
  const isToday = selectedDate === today;

  const handleStatusChange = useCallback((appointmentId: string, newStatus: Appointment['estado']) => {
    setLocalAppointments(prev => prev.map(a => a.id === appointmentId ? { ...a, estado: newStatus } : a));
    setDetailAppointment(prev => prev && prev.id === appointmentId ? { ...prev, estado: newStatus } : prev);
  }, []);
  ...
  const handleMoveAppointment = useCallback((appointmentId: string, newDate: string, newHoraInicio: string, newHoraFin: string) => {
    setLocalAppointments(prev => prev.map(a =>
      a.id === appointmentId
        ? { ...a, fecha: newDate, horaInicio: newHoraInicio, horaFin: newHoraFin }
        : a
    ));
    // Update detail modal if open for this appointment
    setDetailAppointment(prev =>
      prev && prev.id === appointmentId
        ? { ...prev, fecha: newDate, horaInicio: newHoraInicio, horaFin: newHoraFin }
        : prev
    );
  }, []);
  ...
  const getAppsForWeekDate = useCallback((date: string) => {
    let apps = getAppsByDate(localAppointments, date);
    if (sucursalActualId) apps = apps.filter(a => a.sucursalId === sucursalActualId);
    if (urlPatientId) apps = apps.filter(a => a.patientId === urlPatientId);
    if (filterDoctor) apps = apps.filter(a => a.doctorId === filterDoctor);
    if (filterSpecialty) apps = apps.filter(a => a.especialidad === filterSpecialty);
    ...
    if (filterSearch.trim()) {
      const q = filterSearch.toLowerCase();
      apps = apps.filter(a => a.patientName.toLowerCase().includes(q) || a.doctorName.toLowerCase().includes(q));
    }
    return apps;
  }, [localAppointments, filterDoctor, filterSpecialty, filterStatus, filterSearch, urlPatientId, sucursalActualId]);

  const headerDateText = view === 'month'
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between mb-2 shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="hidden md:flex items-center gap-1 pl-3 border-l border-secondary-200/70">
            {[
              { label: 'Total', value: stats.total, icon: 'ri-calendar-line', color: 'text-foreground-500', bg: 'bg-foreground-50' },
              { label: 'Atendidas', value: stats.atendidas, icon: 'ri-check-double-line', color: 'text-emerald-500', bg: 'bg-emerald-50' },
              { label: 'En curso', value: stats.enCurso, icon: 'ri-time-line', color: 'text-accent-500', bg: 'bg-accent-50' },
              { label: 'Pend.', value: stats.confirmadas, icon: 'ri-hourglass-line', color: 'text-primary-500', bg: 'bg-primary-50' },
              { label: 'Cancel.', value: stats.canceladas, icon: 'ri-close-circle-line', color: 'text-red-500', bg: 'bg-red-500/10' },
            ].map((stat) => (
              <div key={stat.label} className="flex items-center gap-1 px-1.5 py-1 rounded-md shrink-0">
                <span className={`w-5 h-5 flex items-center justify-center rounded ${stat.bg} ${stat.color}`}>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button variant="secondary" size="sm" onClick={() => setConfigOpen(true)}>
            <i className="ri-settings-3-line"></i> <span className="hidden sm:inline">Configuración</span>
          </Button>
          <Button variant="primary" size="sm" onClick={() => { setDefaultScheduleTime(undefined); setLockDateTime(false); setDefaultConsultorioId(undefined); setNewModalOpen(true); }}>
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold text-primary-800 truncate">
                Viendo solo citas de {filteredPatient.nombre} {filteredPatient.apellidos}
              </p>
              <p className="text-[10px] text-primary-500">Exp. {filteredPatient.expediente} — Los demás pacientes están ocultos</p>
            </div>
          </div>
          <button
            onClick={() => {
      ...
      <div className="flex items-center gap-2 mb-2 flex-wrap shrink-0">
        ...
        <div className="flex items-center gap-0.5 bg-secondary-100 rounded-lg p-0.5 shrink-0" role="group" aria-label="Navegación de fecha">
          <button
            type="button"
            aria-label={view === 'month' ? 'Mes anterior' : view === 'day' ? 'Día anterior' : 'Semana anterior'}
            onClick={() => {
              if (view === 'month') handlePrevMonth();
              else setSelectedDate(view === 'day' ? addDays(selectedDate, -1) : addDays(selectedDate, -7));
            }}
          ...
          <button
            ...
            className={`px-2 py-1 text-[11px] font-medium rounded-md transition-base cursor-pointer whitespace-nowrap ${isToday && view !== 'month' ? 'bg-primary-500 text-white' : view === 'month' && currentMonth === todayDate.getMonth() && currentYear === todayDate.getFullYear() ? 'bg-primary-500 text-white' : 'text-foreground-600 hover:text-foreground-900 hover:bg-secondary-200'}`}
          >
            Hoy
          </button>
          <button
            type="button"
            aria-label={view === 'month' ? 'Mes siguiente' : view === 'day' ? 'Día siguiente' : 'Semana siguiente'}
            onClick={() => {
        ...
        {/* Time granularity */}
        <div className="flex items-center gap-0.5 bg-secondary-100 rounded-lg p-0.5 shrink-0" role="group" aria-label="Intervalo de tiempo">
          {[
        ...
        >
          ...
          {activeFiltersCount > 0 && (
            <span className="ml-0.5 w-4 h-4 flex items-center justify-center rounded-full bg-primary-500 text-white text-[10px] font-bold" aria-label={`${activeFiltersCount} filtros activos`}>
              {activeFiltersCount}
            </span>
          )}
        </button>
        ...
        {activeFiltersCount > 0 && (
          <button
            ...
            onClick={() => { setFilterDoctor(''); setFilterSpecialty(''); setFilterStatus(''); setFilterSearch(''); }}
      ...
      {showFilters && (
        <div id="filtros-citas" role="region" aria-label="Filtros de citas" className="flex items-center gap-2 mb-2 flex-wrap shrink-0">
          <Select
            placeholder="Médico"
            aria-label="Filtrar por médico"
            options={[{ value: '', label: 'Todos los médicos' }, ...doctors.filter(d => d.status === 'activo').map(d => ({ value: d.id, label: d.nombre }))]}
            value={filterDoctor}
          ...
          <Select
            ...
            options={[{ value: '', label: 'Todas' }, ...specialties.map(s => ({ value: s.nombre, label: s.nombre }))]}
      ...
      {/* ── Status Legend ── */}
      {view !== 'month' && (
        <div className="flex items-center gap-1.5 mb-2 flex-wrap shrink-0 overflow-x-auto" role="list" aria-label="Leyenda de estados de cita">
          {Object.entries(statusConfig).filter(([k]) => k !== 'disponible').map(([key, cfg]) => (
            <div key={key} className="flex items-center gap-1 shrink-0" role="listitem">
              <span className={`w-2 h-2 rounded-full ${cfg.borderColor.replace('border-l-', 'bg-').replace('-400', '-500').replace('-300', '-400')}`} aria-hidden="true"></span>
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold text-accent-800">
                Arrastra para desplazar el calendario
              </p>
              <p className="text-[10px] text-accent-600">
                Mantén clic sostenido y mueve el ratón en cualquier dirección. También puedes usar la rueda del ratón.
              </p>
            </div>
          </div>
          <button
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {view === 'day' ? (
          ...
          <AgendaListView
            ...
            onMarkAttendance={(a) => handleStatusChange(a.id, 'llego')}
            onLinkTriage={(a) => handleStatusChange(a.id, 'en_triage')}
          />
        ) : (
          <MonthView
            year={currentYear}
      ...
      <AgendaConfigModal
        ...
        reglasBloqueo={reglasBloqueo}
        setReglasBloqueo={setReglasBloqueo}
